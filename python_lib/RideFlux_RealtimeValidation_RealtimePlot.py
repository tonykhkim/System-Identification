...
with torch.no_grad():
    ...
    for i in range(4):
        line1, = axs[i // 2, i % 2].plot([], [], 'r-', label='High')  # 빈 데이터([])를 가진 초기 line객체를 생성
        line2, = axs[i // 2, i % 2].plot([], [], 'b-', label='Low')
    ...
    def update(frame):
        ...
        high_pred_list.extend(high_pred.cpu().numpy().tolist())  # list.extend(iterable)는 리스트 끝에 가장 바깥쪽 iterable의 모든 항목을 넣는다.
        low_pred_list.extend(low_pred.cpu().numpy().tolist())
        state_list.extend(state.cpu().numpy().tolist())

        if len(high_pred_list) > 500:
            high_pred_list.pop(0)  # pop(0)는 리스트의 0번째 원소를 리턴하고 삭제
            low_pred_list.pop(0)
            state_list.pop(0)
        ...
        if len(high_pred_list) >= 500:
            ...
            high_pred_subarray = high_pred_array[-500:]  # [-500:]는 배열의 끝에서부터 500개 요소를 가져오라는 의미
            ...
            for i in range(4):
                print("Plotting high_x[i]: ", high_x[i])
                lines[i][0].set_data(high_x[i], high_y[i])
                lines[i][1].set_data(low_x[i], low_y[i])      
                
        return [line for pair in lines for line in pair]

    ani = FuncAnimation(fig, update, frames=NetworkInput_loader, init_func=init, blit=True)
    plt.show()